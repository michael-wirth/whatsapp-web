...
class SecurityServiceProvider implements ServiceProviderInterface, EventListenerProviderInterface, ControllerProviderInterface, BootableProviderInterface
{
    ...
    public function register(Container $app)
    {
        ...
        $app['security.firewall_map'] = function ($app)
        {
            ...
            foreach ($configs as $name => $config)
            {
                ...
                $map->add($requestMatcher, array_map(function ($listenerId) use ($app, $name)
                {
                ...
                , $config['listeners']) , $config['protected'] ? $app['security.exception_listener.' . $name] : null);
            }
            return $map;
        };
        ...
        $app['security.last_error'] = $app->protect(function (Request $request)
        {
            ...
            {
                $message = $session->get(Security::AUTHENTICATION_ERROR)
                    ->getMessage();
    ...
    public function connect(Application $app)
    {
        ...
        foreach ($this->fakeRoutes as $route)
        {
            ...
            $controllers->$method($pattern)->run(null)
                ->bind($name);
        }
        return $controllers;
    }
